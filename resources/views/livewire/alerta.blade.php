<div class="container mx-auto px-8">
    ...
    <h2 class="text-center text-4xl font-bold mb-8 relative text-yellow-500">
        <span class="italic text-gray-900">ALERTAS</span>
    ...
    <div class="flex justify-between items-center mb-6">
        <button wire:click="openModal()" class="bg-red-600 text-white font-bold px-6 py-3 rounded-md shadow-lg transition duration-300 hover:bg-red-700 text-lg">NUEVA ALERTA</button>
        ...
        <table class="min-w-full bg-white border border-gray-300">
            <thead>
                <tr class="bg-red-600 text-white">
                    <th class="px-4 py-2 border">Tipo de Alerta</th>
                    <th class="px-4 py-2 border">Mensaje</th>
                    <th class="px-4 py-2 border">Leída</th>
            ...
            <tbody>
                @forelse($alertas as $item)
                    <tr class="text-center">
                        <td class="px-4 py-2 border">{{ $item->tipo_alerta }}</td>
                        <td class="px-4 py-2 border">{{ $item->mensaje }}</td>
                        <td class="px-4 py-2 border">{{ $item->leida }}</td>
                        <td class="px-4 py-2 border">
                            <button wire:click.prevent="editar({{$item->id_alerta}})" class="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 transition">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-2 text-center text-gray-600">No hay alertas registradas.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
